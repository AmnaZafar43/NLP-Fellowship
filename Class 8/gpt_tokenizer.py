...
Original file is located at
    https://colab.research.google.com/drive/1LZgFmdgu9D9zJ43FlvWyir_hdQfNZdD-
"""

# text is from rathan reed programmer's intro to unicode
text = 'Ｕｎｉｃｏｄｅ! 🅤🅝🅘🅒🅞🅓🅔‽ 🇺‌🇳‌🇮‌🇨‌🇴‌🇩‌🇪! 😄 The very name strikes fear and awe into the hearts of programmers worldwide. We all know we ought to “support Unicode” in our software (whatever that means—like using wchar_t for all the strings, right?). But Unicode can be abstruse, and diving into the thousand-page Unicode Standard plus its dozens of supplementary annexes, reports, and notes can be more than a little intimidating. I don’t blame programmers for still finding the whole thing mysterious, even 30 years after Unicode’s inception.'
# raw bytes
tokens = text.encode("utf-8")
# convert a list of integers in range 0-225 for convenience
...
# now we gonna swap elements to get new id of 256
top_pair = max(stats, key=stats.get)
print(top_pair)

# now we are merging most common pairs
def merge(ids,pair,idx):
  # replace all consecutive occurences of pair with new token idx
  new_ids = []
  i = 0
  while i < len(ids):
...
print("Length of token 2:", len(token2))
...
# encoding(Given a string, what are the tokens?)
def encode(text):
